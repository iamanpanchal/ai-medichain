  ...
  return {
    ...
    configureServer(server) {
      server.middlewares.use('/api/ai-chat', async (req, res) => {
        ...
        if (!apiKey) { res.statusCode = 503; res.end('ANTHROPIC_API_KEY is not configured.'); return; }
        let body = '';
        ...
        try {
          ...
          res.end();
        } catch { res.statusCode = 500; res.end('The AI service request failed.'); }
      });
    },
  };
}
...
export default defineConfig(({ mode }) => {
  ...
  return {
  plugins: [react(), tailwindcss(), viteSingleFile(), anthropicDevProxy(env.ANTHROPIC_API_KEY)],
>>>>>>> 2e33143 (add chatbot)
  resolve: {
    alias: {
